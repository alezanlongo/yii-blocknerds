...
class UserCollectionFormTest extends Unit
{
    ...
    public function testCreateCollection() {
        $userCollectionFormMock = $this->make(UserCollectionForm::class);
        $userCollectionFormMock->collection = $this->jsonCollectionCreate;
        $userCollectionFormMock->name = 'test';

        $user = $this->tester->grabRecord('common\models\User', [
            'id' => 1,
        ]);
        $this->assertTrue($userCollectionFormMock->validate());
        $this->assertTrue($userCollectionFormMock->createCollection($user));
    }

    public function testEmptyDataCreateCollection() {
        $userCollectionFormMock = $this->make(UserCollectionForm::class);
        $userCollectionFormMock->collection = '';
        $userCollectionFormMock->name = 'test';

        $user = $this->tester->grabRecord('common\models\User', [
            'id' => 1
        ]);
        $this->assertFalse($userCollectionFormMock->validate());
        $this->assertFalse($userCollectionFormMock->createCollection($user));
    }

    public function testInvalidDataCreateCollection() {
        $userCollectionFormMock = $this->make(UserCollectionForm::class);
        $userCollectionFormMock->collection = $this->jsonCollectionCreate . 'dasd addadas dasd asdfaefa f';
        $userCollectionFormMock->name = 'test';
        ...
        $userCollectionFormMock->createCollection($user);
    }
    ...
    public function testUpdateCollection() {
        ...
        $userCollectionFormMock = $this->construct(UserCollectionForm::class, [$collectionModel]);
        $this->assertTrue($userCollectionFormMock->validate());
        $this->assertTrue($userCollectionFormMock->updateCollection());
    }

    public function testInvalidDataUpdateCollection() {

        $collectionModel = $this->tester->grabRecord('common\models\UserCollection', [
            'id' => 1
        ]);
        /* @var $userCollectionFormMock UserCollectionForm */
        $userCollectionFormMock = $this->construct(UserCollectionForm::class, [$collectionModel]);
        $userCollectionFormMock->collection = '123123';
        $this->assertTrue($userCollectionFormMock->validate());
        $this->expectExceptionObject(new InvalidArgumentException('invalid collection data'));
        $this->assertTrue($userCollectionFormMock->updateCollection());
    }

    public function testDeleteUserCollection() {
        $collectionModel = $this->tester->grabRecord('common\models\UserCollection', [
            'id' => 1
        ]);
        $this->assertNotNull($collectionModel);
        $collectionModel->delete();
        $collectionModelAfter = $this->tester->grabRecord('common\models\UserCollection', [
            'id' => 1
        ]);
        $this->assertNull($collectionModelAfter);
    }
